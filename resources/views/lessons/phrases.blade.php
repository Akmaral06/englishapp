...
<div class="card">
    <div style="display:flex; align-items:center; gap:15px; flex-wrap:wrap;">
        ...
        <h1 style="color:var(--primary); margin:0;">{{ __('app.phrases_title') }}</h1>
    </div>
    <p style="color:#666; margin-top:10px;">{{ __('app.phrases_desc') }}</p>
</div>
...
<div class="card">
    <h2 style="color:{{ $section['color'] }}; margin-bottom:15px;">{{ $section['title'] }}</h2>
    <div style="display:flex; flex-direction:column; gap:8px;">
        @foreach($section['phrases'] as $phrase)
        <div style="display:flex; align-items:center; gap:15px; background:{{ $section['bg'] }};
                    padding:12px 16px; border-radius:8px;">
            <span style="flex:1; font-weight:600; color:{{ $section['color'] }};">{{ $phrase[0] }}</span>
            <span style="color:#777; font-size:0.9em;">{{ $phrase[1] }}</span>
        </div>
        @endforeach
    </div>
</div>
...
<div class="card" style="background:#e8f5e9; text-align:center;">
    <h3 style="color:#2e7d32;">{{ __('app.phrases_tip') }}</h3>
    <p style="color:#555;">{{ __('app.phrases_tip_t') }}</p>
    <a href="/exercises" class="btn" style="background:#4caf50; margin-top:10px;">{{ __('app.phrases_go_ex') }}</a>
</div>
