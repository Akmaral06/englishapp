...
<div class="card">
    <h1 style="color:var(--primary);">{{ __('app.ex_title') }}</h1>
    <p style="color:#666;">{{ __('app.ex_sub') }}</p>
</div>
...
<div style="display:flex; gap:8px; flex-wrap:wrap; margin-bottom:20px;">
    ...
    <button onclick="showTab('{{ $tab['id'] }}')" id="tab-{{ $tab['id'] }}"
        style="padding:10px 20px; border:2px solid {{ $tab['color'] }}; border-radius:8px; cursor:pointer;
               font-weight:600; background:white; color:{{ $tab['color'] }}; font-size:0.95em; transition:all 0.2s;">
        {{ $tab['label'] }}
    </button>
    @endforeach
</div>
...
<div id="ex-alphabet" class="ex-section">
    <div class="card">
        ...
        @endphp
        @foreach($alphaQ as $qi => $q)
        <div style="background:#f0f4ff; border-radius:10px; padding:18px; margin-bottom:15px;">
            <p style="font-weight:600; font-size:1.1em;">{{ $qi+1 }}. What is the name of the letter <strong style="font-size:1.6em; color:#1565c0;">{{ $q['letter'] }}</strong>?</p>
            <div style="display:flex; gap:10px; flex-wrap:wrap; margin-top:10px;">
                @foreach($q['options'] as $oi => $opt)
                <button onclick="checkAlpha(this, {{ $qi }}, {{ $oi }}, {{ $q['correct'] }})"
            ...
            <div id="alpha-result-{{ $qi }}" style="margin-top:8px; font-weight:600;"></div>
        </div>
        @endforeach
    </div>
</div>
...
<div id="ex-vocab" class="ex-section" style="display:none;">
    <div class="card">
        ...
        @endphp
        @foreach($vocabQ as $qi => $q)
        <div style="background:#fff8f0; border-radius:10px; padding:18px; margin-bottom:15px;">
            <p style="font-weight:600; font-size:1.05em;">{{ $qi+1 }}. {{ $q['q'] }}</p>
            <div style="display:flex; gap:10px; flex-wrap:wrap; margin-top:10px;">
                @foreach($q['options'] as $oi => $opt)
                <button onclick="checkVocab(this, {{ $qi }}, {{ $oi }}, {{ $q['correct'] }})"
            ...
            <div id="vocab-result-{{ $qi }}" style="margin-top:8px; font-weight:600;"></div>
        </div>
        @endforeach
    </div>
</div>
...
<div id="ex-grammar" class="ex-section" style="display:none;">
    <div class="card">
        ...
        <div style="background:#f0fff4; border-radius:10px; padding:18px; margin-bottom:15px;">
            ...
            <div id="gram-result-{{ $qi }}" style="margin-top:8px; font-weight:600;"></div>
        </div>
        @endforeach
    </div>
</div>
...
<div id="ex-phrases" class="ex-section" style="display:none;">
    <div class="card">
        ...
        <div style="background:#fdf0ff; border-radius:10px; padding:18px; margin-bottom:15px;">
            ...
            <div id="phrase-result-{{ $qi }}" style="margin-top:8px; font-weight:600;"></div>
        </div>
        @endforeach
    </div>
</div>
